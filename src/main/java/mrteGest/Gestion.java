    ...
    public Gestion()
    {
        ...
        try
        {
            ...
            while((carLu = lecteur.read()) != -1)
            {
                switch(carLu)
                {
                    case '$':	if(numAttribut == 3 || numAttribut == 12)
                    {
                        if( !attribut.equals("") )
                        {
                            try
                            ...
                            {
                                JOptionPane.showMessageDialog(null, "La date de la ligne "+String.valueOf(numAttribut)+" du fichier n'est pas au format JJ/MM/YYYY ("+attribut+").", "Erreur", JOptionPane.ERROR_MESSAGE);
                                System.exit(0);
                            }
                        }
                        else
                    ...
                    numAttribut++;
                    break;
                    case 13:listeLigne.add(listeAttributs);listeAttributs = new ArrayList();attribut = "";lecteur.read();numAttribut = 0;
                    break;
                    default:attribut += (char) carLu;
                }//switch
            }//while
            lecteur.close();
            attribut = "";
            ...
            while((carLu = lecteur.read()) != -1)
            {
                switch(carLu)
                {
                    ...
                    attribut = "";
                    break;
                    case 13:attribut = "";lecteur.read();numLigne++;
                    break;
                    default:attribut += (char) carLu;
                }//switch
            }//while
                ...
                while(reponse ==0)
                {
	                try {
			            ...
			            reponse = JOptionPane.showOptionDialog(null, "Echec de sauvegarde des données modifiées, voulez vous réessayer?\n" +
			            		"Si oui attendez 30 secondes avant de réessayer, le serveur est peut être en train de dormir...", "Attention", JOptionPane.DEFAULT_OPTION, JOptionPane.WARNING_MESSAGE,null, options, options[0]);
					}
                }
                dialog.dispose();
				System.exit(0);
...
//                JOptionPane.showMessageDialog(null, "Aucun membre trouvé.", "Info", JOptionPane.INFORMATION_MESSAGE);
        ...
        else if(e.getSource() == bAjout)
        {
            ...
            else
            {
                if(rAgeInter.isSelected())
                    JOptionPane.showMessageDialog(null, "Pour l'ajout, il faut positionner le bouton sur la zone de texte de l'année de naissance.", "Attention", JOptionPane.WARNING_MESSAGE);
                else
                {
                    if(!tNumLicense.getText().equals(""))
                        for(int i = 0;i < listeLigne.size() && !dejaPresent;i++)
                        {
                            ...
                            {
                                JOptionPane.showMessageDialog(null, "Cet enregistrement existe déjà.\nSéléctionnez le si vous voulez le modifier.", "Attention", JOptionPane.WARNING_MESSAGE);
                                dejaPresent = true;
                            }
                        }
                    else
                    {
                        ...
                        int fauxNum = -1;
                        int leNumLic = 0;
                        reponse = JOptionPane.showOptionDialog(null, "Si vous ne rentrez pas de numéro de licence, le programme va en genérer un faux automatiquement.\nVous pourrez le modifier à tous moments.\nVoulez vous continuer?", "Attention", JOptionPane.DEFAULT_OPTION, JOptionPane.WARNING_MESSAGE,null, options, options[0]);
                        if(reponse == 0)
                        {
                            ...
                            if(tNumLicense.isEnabled())
                            {
                                ...
                                if(tNumLicense.getText().charAt(0) != '-')
                                    for(int k = 0;k < listeLigne.size() && !dejaPresent;k++)
                                    {
                                        ...
                                        if(((String) listeAttributs.get(0)).equals((tNumLicense.getText()).trim()))
                                        {
                                            JOptionPane.showMessageDialog(null, "Il existe déjà un membre avec ce numéro de licence.", "Attention", JOptionPane.WARNING_MESSAGE);
                                            dejaPresent = true;
                                        }
                                    }
                            }
        ...
        else if(e.getSource() == bSuppression)
        {
            ...
            if(modeModifSupp)
            {
                reponse = JOptionPane.showOptionDialog(null, "Etes vous certain de vouloir supprimer cet enregistrement?", "Attention", JOptionPane.DEFAULT_OPTION, JOptionPane.WARNING_MESSAGE,null, options, options[0]);
                if(reponse == 0)
                {
                    for(i = 0;i < listeLigne.size() && !trouve;i++)
                    {
                    ...
                    {
                        listeLigne.remove(i-1);
                        JOptionPane.showMessageDialog(null, "Suppression du membre N°"+(String) listeAttributs.get(0)+" effectuée.", "Info", JOptionPane.INFORMATION_MESSAGE);
                        bRaz.doClick();
                        bRecherche.doClick();
                    }
                }//if oui
                ...
                JOptionPane.showMessageDialog(null, "Il faut séléctionner un membre dans le tableau\navant de pouvoir supprimer.", "Attention", JOptionPane.WARNING_MESSAGE);
        ...
        else if(e.getSource() == bChangeAnnee)
        {
            ...
            { "Oui", "Non" };
            int reponse, i;
            reponse = JOptionPane.showOptionDialog(null, "Tous les nouveaux membres vont devenir ancien\net tous les inscrit vont être désinscrit?\nL'ancien fichier de données sera sauvegardé sous le nom de : "+"data_"+formatDateFic.format(new Date())+".txt", "Attention", JOptionPane.DEFAULT_OPTION, JOptionPane.WARNING_MESSAGE,null, options, options[0]);
            if(reponse == 0)
            {
            ...
            if((tNumLicense.getText()).equals("") || ((String) listeAttributs.get(0)).toUpperCase().indexOf(tNumLicense.getText().toUpperCase()) != -1)
                if((tNom.getText()).equals("") || ((String) listeAttributs.get(1)).toUpperCase().indexOf(tNom.getText().toUpperCase()) != -1)
                    if((tPrenom.getText()).equals("") || ((String) listeAttributs.get(2)).toUpperCase().indexOf(tPrenom.getText().toUpperCase()) != -1)
                    {
                        ...
                        if(ageOk == true)
                            if((tAdresse.getText()).equals("") || ((String) listeAttributs.get(4)).toUpperCase().indexOf(tAdresse.getText().toUpperCase()) != -1)
                                if((tCodP.getText()).equals("") || ((String) listeAttributs.get(5)).toUpperCase().indexOf(tCodP.getText().toUpperCase()) != -1)
                                    if((tVille.getText()).equals("") || ((String) listeAttributs.get(6)).toUpperCase().indexOf(tVille.getText().toUpperCase()) != -1)
                                        if((tTel.getText()).equals("") || ((String) listeAttributs.get(7)).toUpperCase().indexOf(tTel.getText().toUpperCase()) != -1)
                                            if(((String) jLic.getSelectedItem()).equals("") || ((String) listeAttributs.get(8)).toUpperCase().indexOf( ((String) jLic.getSelectedItem()).toUpperCase() ) != -1)
                                                if((tPrixLic.getText()).equals("") || ((String) listeAttributs.get(9)).toUpperCase().indexOf(tPrixLic.getText().toUpperCase()) != -1)
                                                    if((tAdhePrix.getText()).equals("") || ((String) listeAttributs.get(10)).toUpperCase().indexOf(tAdhePrix.getText().toUpperCase()) != -1)
                                                    {
                                                        if((Date) listeAttributs.get(12) != null)
                                                            dateAttribut = formatDate.format( (Date) listeAttributs.get(12) );
                                                        else
                                                            dateAttribut = "";
                                                        if((tDateEdition.getText()).equals("") || (Date) listeAttributs.get(12) != null &&
                                                        		( 
	                                                        		(jCompDate.getSelectedItem().equals("=") && dateAttribut.toUpperCase().indexOf(tDateEdition.getText().toUpperCase()) != -1) ||
	                                                        		(jCompDate.getSelectedItem().equals(">") && ((Date) listeAttributs.get(12)).after(formatDate.parse(tDateEdition.getText())))||
	                                                        		(jCompDate.getSelectedItem().equals("<") && ((Date) listeAttributs.get(12)).before(formatDate.parse(tDateEdition.getText())))
                                                        		))
                                                            if(((String) jCategorie.getSelectedItem()).equals("") || ((String) listeAttributs.get(11)).toUpperCase().indexOf( ((String) jCategorie.getSelectedItem()).toUpperCase() ) != -1)
                                                                if(rNouvAncT.isSelected() || (((String) listeAttributs.get(13)).equals("oui") && rNouvAncN.isSelected()) || (((String) listeAttributs.get(13)).equals("non") && rNouvAncA.isSelected()))
                                                                    if(rInscritT.isSelected() || (((String) listeAttributs.get(14)).equals("oui") && rInscritI.isSelected()) || (((String) listeAttributs.get(14)).equals("non") && rInscritN.isSelected()))
                                                                        if(rAssuranceT.isSelected() || (((String) listeAttributs.get(15)).equals("oui") && rAssuranceO.isSelected()) || (((String) listeAttributs.get(15)).equals("non") && rAssuranceN.isSelected()))
                                                                    {
