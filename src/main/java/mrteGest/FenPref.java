...
public class FenPref extends JFrame implements ActionListener {
    ...
    JCheckBox cNumLicense, cNom, cPrenom, cDatN, cAdresse, cCodP, cVille, cTel, cTypLic, cPrixLic, cAdhePrix, cCat, cNouvAnc, cInscrit, cAssurance,
            cDateEdition, cMail, cAdresse2;
    ...
    private FenPref(Gestion gestion, String titre, String libelle) {
        ...
        JPanel lePanelH = new JPanel(new GridLayout(11, 4));
        ...
        JLabel lAdresse2 = new JLabel("Complément d'adresse");
        ...
        lePanelH.add(lAdresse2);
        lePanelH.add(cAdresse2);
        lePanelH.add(lMail);
        lePanelH.add(cMail);
        lePanelH.add(lCodP);
        lePanelH.add(cCodP);
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == bVoir) {
            ...
            if (!cAdresse2.isSelected()) {
                newEnTete.remove(16 - j);
                for (i = 0; i < newRangees.size(); i++) {
                    rangee = (Vector) ((Vector) newRangees.elementAt(i)).clone();
                    rangee.remove(16 - j);
                    newRangees.remove(i);
            ...
            if (newEnTete == null || newEnTete.size() == 0)
                JOptionPane.showMessageDialog(null, "Il faut effectuer l'apercu d'au moins une colonne avant de pouvoir imprimer.", "Attention",
                        JOptionPane.WARNING_MESSAGE);
            else {
                try {
                    pTableView.tableView.print(JTable.PrintMode.FIT_WIDTH, new MessageFormat("MRTE CLUB N°20"),
                            new MessageFormat("Page {0} / " + newRangees.size() + " licences = " + laGestion.prixTotalLic + "€"));
            ...
            if (newEnTete == null || newEnTete.size() == 0)
                JOptionPane.showMessageDialog(null, "Il faut effectuer l'apercu d'au moins une colonne avant de pouvoir générer un fichier Excel.",
